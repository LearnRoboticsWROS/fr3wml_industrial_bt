#
# FR3WML bringup launch for industrial_bt_framework (headless — no Groot2).
#
# This launch file is a thin wrapper over the framework:
#   - Starts the MoveIt stack (move_group, robot_state_publisher, RViz,
#     ros2_control controllers) via MoveItConfigsBuilder on the FR3WML
#     moveit config package.
#   - Includes the framework's bt_runner.launch.py, which starts
#     scene_manager_node + bt_runner_node with the FR3WML configs and
#     the bottle/capsule BT tree.
#
# NOT launched here (user runs them in separate terminals):
...
def generate_launch_description():
    ...
    default_bt_tree        = os.path.join(pkg_share, "bt_trees", "bottle_capsule_task.xml")
    ...
    # ── MoveIt stack (identical to the monolith's launch) ────────────────────
    moveit_config = (
    ...
    # ── Scene manager + BT runner (headless) ─────────────────────────────────
    # bt_runner_node internally builds a MoveGroupInterface, so the MoveIt
    # robot_description / semantic / kinematics params must reach it directly
    # on the node (topic fallback is unreliable during bringup).
    scene_manager = Node(
